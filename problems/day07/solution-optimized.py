...
def progress_light_part2(current_light, next_row) -> list:
    # print(f"current_light: {current_light}")
    # print(f"next_row: {next_row}")
    updated_light: List[Optional[int]] = [0] * len(current_light)

    for i in range(1, len(current_light) - 1):
        obstacle = next_row[i]
        value = current_light[i]
        if obstacle == ".":
            updated_light[i] += value

        elif obstacle == "^":
            updated_light[i - 1] += value
            updated_light[i + 1] += value
...
def run_system_part2_new(text_in):
    system = text_in[1:]
    loc_start, system_width, system_length = find_system_params(text_in)
    initial_light: List[Optional[int]] = [0] * system_width
    initial_light[loc_start] = 1
    current_light = []

    for next_row in system:
        current_light = initial_light.copy()
        updated_light = progress_light_part2(current_light, next_row)
        initial_light = updated_light.copy()

    return sum_list(current_light)
...
def solve_part2(text_in):
    total_world_count = run_system_part2_new(text_in)
